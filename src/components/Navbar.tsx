...
type NavbarItems = {
  data: string;
  link: string; // nombre de la sección
};
...
function Navbar({ items, onNavigate }: Props) {
  const [active, setActive] = useState<string>(""); // 👈 guarda el botón activo

  const handleClick = (section: string) => {
    setActive(section); // actualiza botón activo
    onNavigate(section); // ejecuta tu scroll o acción
  };
   useEffect(() => {
    if (items.length > 0) {
      setActive(items[0].link);
      onNavigate(items[0].link); // también puedes hacer que navegue automáticamente
    }
  }, [items]);
  return (
        ...
        {items.map((item, idx) => (
          <button
            ...
            className={`btn btn-ghost transition-all duration-200 
              hover:bg-white/10 hover:border-white/15 hover:backdrop-blur-lg 
              text-base p-0.5 md:text-lg md:p-1 lg:text-xl lg:p-1.5 xl:text-2xl xl:p-2
              ${
                active === item.link
